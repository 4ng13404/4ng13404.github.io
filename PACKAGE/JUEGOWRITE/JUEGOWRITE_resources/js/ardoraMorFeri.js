...
$("#tr_"+indexAttemps.toString()).append('<td class="hidetd"></td>');$("#tr_"+indexAttemps.toString()).append('<td class="green cell" id="scoregr_'+indexAttemps.toString()+'">0</td>');$("#tr_"+indexAttemps.toString()).append('<td class="yellow cell" id="scoreye_'+indexAttemps.toString()+'">0</td>');$("#tr_"+indexAttemps.toString()).append('<td class="red cell" id="scorere_'+indexAttemps.toString()+'">0</td>');
...
$("#scorere_"+indexAttemps.toString()).text(l_red.toString());
var l_green=0;for (i=0;i<nl;i++) {if (v_correct[i]==v_ans[i]){l_green++;v_ans[i]="|";v_correct[i]="~";
}};
$("#scoregr_"+indexAttemps.toString()).text(l_green.toString());
var l_yellow=0;for (i=0;i<nl;i++) {var le=v_ans[i];if (le!="|"){for (j=0;j<nl;j++) {if (v_correct[j]==le){l_yellow++;v_ans[i]="|";le="|";v_correct[j]="~";
}}}};
$("#scoreye_"+indexAttemps.toString()).text(l_yellow.toString());
if (CWWords(words[indexWords]).localeCompare(ans)==0){correct=true;};var idSetTimeout;for (i=0;i<nl;i++){$("#td"+i.toString()+"_"+indexAttemps.toString()).css("width","40px");$("#td"+i.toString()+"_"+indexAttemps.toString()).css("height","48px");$("#td"+i.toString()+"_"+indexAttemps.toString()).removeClass("activeCell");$("#td"+i.toString()+"_"+indexAttemps.toString()).text($("#C"+i.toString()+"_"+indexAttemps.toString()).val());$("#C"+i.toString()+"_"+indexAttemps.toString()).remove();}
var c = $("#ardoraActCanvas"), ctx = c[0].getContext("2d");ctx.canvas.height=$("#ardoraAct").height()+35;if (correct) {score=score+scoreInc;timeAct=timeAct+timeBon;attempts=0;successes++;indexWords++;if (indexWords==words.length){$("#ardoraActCanvas").attr("aria-label", messageOk);showMessage("Ok");$("#buttonOk").remove();}
else{idSetTimeout = setTimeout(function (element) {indexAttemps=0;paintboard();clearTimeout(idSetTimeout);},1500, $(this));}} else {attempts++;score=score-scoreDec;indexAttemps++;
idSetTimeout=setTimeout(function (element) {if (attempts >= attemptsMax) {$("#ardoraActCanvas").attr("aria-label", messageAttempts);$("td").addClass("cell");showMessage("Attempts");} else {$("#ardoraActCanvas").attr("aria-label", messageError);if (messageError.trim().length>0){showMessage("Error");};paintboard()};clearTimeout(idSetTimeout);},500, $(this));}}
